...
void screen_buffer_append_debug_information(struct abuf *abptr) {
  ...
  ab_append(DEB.debug_string, buf,
...
void editor_draw_rows(struct abuf *abptr) {
  for (unsigned int nrows = 0; nrows < E.screen_rows;
    ...
    if (nrows == editor_get_cursor_screen_pos_y()) {
      lineNumber =
          editor_get_cursor_textbuf_pos_y() + 1; // linenumber counts from 1
      ...
      if (strnlen_s(leftMargin, 256) < E.left_margin_size) {
        ...
        for (size_t i = 0; i < E.left_margin_size - strlen(leftMargin) - 1;
             i++) {
          ab_append(abptr, " ", 1);
        }
      }
    ...
    if (n_rows_to_draw >= TEXTBUF.size) {
      ...
      const unsigned int xoffset =
          E.offset_x >= stringlen ? stringlen : E.offset_x;
      temp += xoffset;
...
void editor_scroll_down(void) { E.offset_y++; }

void editor_scroll_up(void) { E.offset_y--; }

void editor_scroll_left(void) { E.offset_x--; }

void editor_scroll_right(void) { E.offset_x++; }
...
void editor_move_cursor_to_end_of_line(textbuf *txb, unsigned int y) {
  ...
  editor_cursor_x_to_textbuf_pos(textbuf_get_nth_line_length(txb, y));
  return;
}
...
int editor_move_cursor(int key) {
  ...
  case KEY_PAGE_UP:
    if (E.cursor_textbuf_pos_y > 0) {
      if (editor_get_cursor_screen_pos_y() <= 0)
        --E.offset_y;
  ...
  case KEY_PAGE_DOWN:
    if (E.cursor_textbuf_pos_y < TEXTBUF.size - 1) {
      ...
      if (editor_get_cursor_screen_pos_y() >= E.screen_rows - 1)
        ++E.offset_y;
      E.cursor_textbuf_pos_y++;
    }
    break;
  ...
  editor_confine_cursor_position(&TEXTBUF, editor_get_cursor_textbuf_pos_x(),
                                 editor_get_cursor_textbuf_pos_y());
  return 0;
}
...
int editor_get_cursor_textbuf_pos_x(void) { return E.cursor_textbuf_pos_x; }

int editor_get_cursor_textbuf_pos_y(void) { return E.cursor_textbuf_pos_y; }

// See if the cursor can be moved further to write
// with current cursro textbuf postion and textbuffer
int editor_cursor_movable_to_right(textbuf *txb, unsigned int cursorTxbPosX,
